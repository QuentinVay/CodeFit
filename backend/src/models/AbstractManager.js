...
class AbstractManager {
  ...
  // Method to read a table record by its ID
  async read(id) {
  ...
  // Method to read all table records
  async readAll() {
    // Performing a database query to select all records from the table table
    const [rows] = await this.database.query(`select * from ${this.table}`);
    // Returning all rows
    return rows;
  }

  // Method to edit/update a table record by ID
  async edit(body, id) {
  ...
  // Method to add/insert a new table record
  async add(body) {
    // Converting values in the body to an array (stringifying arrays if present)
    const values = isArray(body);
    ...
    // Performing a database query to insert a new record into the table table
    const [rows] = await this.database.query(
